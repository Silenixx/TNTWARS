 ...
 public class TaskRejoindPartieEnCours extends BukkitRunnable {
   ...
   public void run() {
     if (this.joueur.getEquipe() != this.main.Sans_Equipe) {
       ...
       if (this.timer == 35 || this.timer == 10 || this.timer == 5 || this.timer == 4 || this.timer == 3 || this.timer == 2 || this.timer == 1 || this.timer == 0) {
         this.joueur.getPlayer().sendMessage(Constante.Constantes.PluginName + "Vous allez rejoindre la partie dans §5" + this.timer + "§b secondes.");
         if (this.timer == 0) {
           this.joueur.getPlayer().sendMessage(Constante.Constantes.PluginName + "Vous avez rejoind le jeu. Amusez vous bien!");
